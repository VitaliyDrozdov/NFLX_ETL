...
BEGIN
    ...
    SELECT i_OnDate,
            ...
            SUM(credit_amount) * (COALESCE(ex.reduced_course, 1)) as credit_amount_rub,
            SUM(debet_amount) AS debet_amount,
            SUM(debet_amount)  * (COALESCE(ex.reduced_course, 1)) as debet_amount_rub
    FROM  (
        SELECT
        ...
        FROM DS.ft_posting_f p
            JOIN DS.md_accoint_d a on p.credit_account_rk = a.account_rk
        ...
        SELECT
            ...
            p.debet_amount
        FROM DS.ft_posting_f p
            JOIN DS.md_accoint_d a on p.debet_account_rk = a.account_rk
        WHERE i_OnDate = p.oper_date
    ) as turnover
...
-- INSERT INTO "DM".account_balance_f (account_rk, balance_out, balance_out_rub)
-- SELECT
--     '2017-12-31',
--     account_rk,
--     balance_out,
...
-- ON
--     ex.currency_rk = bal.currency_rk
...
LEFT JOIN
    "DS".md_exchange_rate_d ex
ON
    ex.data_actual_date = '2017-12-31' AND ex.currency_rk = bal.currency_rk;
...
BEGIN
    WHILE d <= '2018-01-31' LOOP
        CALL fill_account_turnover_f(d);
        d := d + INTERVAL '1 day';
    END LOOP;
END $$;
